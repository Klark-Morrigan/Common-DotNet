...
- Infrastructure repositories share CI via reusable workflows published by
  `Infrastructure-Common` (`ci-powershell.yml`, `ci-powershell-docker-host.yml`,
...
## Why This Repo
- `Infrastructure-Common`'s de facto scope is PowerShell module CI; its
...
- `DotNet-Common` mirrors the `Infrastructure-Common` naming so each shared
